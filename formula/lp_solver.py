...
class LPSolver:
    ...
    def solve(
        ...
        # Handle empty constraints: if A_ub is empty list, pass empty list
        # If A_ub is None, pass None (linprog will handle it)
        A_ub_list = None
        if A_ub is not None:
            if len(A_ub) == 0:
                A_ub_list = []  # Empty list for no constraints
            else:
        ...
        if b_ub is not None:
            if len(b_ub) == 0:
                b_ub_list = []  # Empty list for no constraints
            else:
                b_ub_list = list(b_ub)
