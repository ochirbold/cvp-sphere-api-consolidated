...
# Import runtime functions (new package path with legacy fallback)
try:
    from formula.core.runtime import run_formula, extract_identifiers, detect_scenario_functions
except ImportError:
    from formula_runtime import run_formula, extract_identifiers, detect_scenario_functions

# Import LP optimization modules (new package path with legacy fallback)
try:
    ...
    try:
        from formula.lp_model_parser import LPModelParser, detect_lp_components
        from formula.lp_matrix_builder_deterministic_complete import LPMatrixBuilder
...
def sort_x_vars_numerically(vars_list: List[str]) -> List[str]:
    """Sort x variables by numeric suffix: x1, x2, ..., x10."""
    ...
    return sorted(vars_list, key=key)
    ...
    try:
        ...
        if result['success'] and result['x'] is not None:
            ...
            if x_vars:
                ...
                optimal_x_values = []
                for x_var in sort_x_vars_numerically(x_vars):
            ...
            x_vars_sorted = sort_x_vars_numerically([var for var in lp_matrices['variables'] if var.startswith('x')])
